  ...
  function cloudFront(file, enc, cb){
    ...
    if(file.isBuffer()){
      try {
        ...
        if(file.path.indexOf('/artists/') !== -1 ) contents = contents.replace('!{artists}', 'active')
        if(file.path.indexOf('/blog/') !== -1 ) contents = contents.replace('!{blog}', 'active')
        if(file.path.indexOf('/releases/') !== -1 ) {
          console.log('editing releases for ', file.path);
          contents = contents.replace('!{releases}', 'active')
        }
